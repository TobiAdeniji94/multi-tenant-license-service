...
from apps.api.exceptions import LicenseKeyNotFoundError, LicenseNotFoundError
...
class LicenseKeyListCreateView(BrandAPIView):
    ...
    def get(self, request):
        ...
        license_keys = LicenseKey.objects.filter(brand=brand).prefetch_related(
            ...
            Prefetch(
                ...
                queryset=Activation.objects.filter(is_active=True),
            ),
        )
    ...
    def post(self, request):
        brand = self.get_brand()
        serializer = LicenseKeyCreateSerializer(
            data=request.data, context={"brand": brand}
        )
        serializer.is_valid(raise_exception=True)
        license_key = serializer.save()

        logger.info(
            f"License key created: {license_key.key[:8]}... for {license_key.customer_email}"
        )
    ...
    def get(self, request, key):
        ...
        try:
            license_key = LicenseKey.objects.prefetch_related(
                Prefetch(
                    "licenses", queryset=License.objects.select_related("product")
                ),
                Prefetch(
                    "licenses__activations",
                    queryset=Activation.objects.filter(is_active=True),
...
class LicenseCreateView(BrandAPIView):
    ...
    def post(self, request, key):
        ...
        serializer = LicenseCreateSerializer(
            data=request.data, context={"brand": brand, "license_key": license_key}
        )
        serializer.is_valid(raise_exception=True)
...
class LicenseDetailView(BrandAPIView):
    ...
    def get(self, request, license_id):
        ...
        try:
            license = (
                License.objects.select_related("product", "license_key")
...
class LicenseRenewView(BrandAPIView):
    ...
    def post(self, request, license_id):
        ...
        logger.info(
            f"License renewed: {license.id} for {serializer.validated_data['days']} days"
        )
