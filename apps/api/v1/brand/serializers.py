...
class ProductSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Product
        fields = ['id', 'name', 'slug', 'description', 'default_seat_limit', 'is_active']
        read_only_fields = ['id']
...
class ActivationSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Activation
        fields = ['id', 'instance_id', 'instance_name', 'is_active', 'activated_at', 'last_check_at']
...
class LicenseSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = License
        fields = [
            'id', 'product', 'product_id', 'status', 'expires_at',
            'seat_limit', 'seats_used', 'seats_available', 'is_valid',
            'activations', 'created_at', 'updated_at'
        ]
...
class LicenseCreateSerializer(serializers.Serializer):
    ...
    def create(self, validated_data):
        ...
        expires_at = validated_data.get('expires_at', timezone.now() + timedelta(days=365))
...
class LicenseKeySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = LicenseKey
        fields = [
            'id', 'key', 'customer_email', 'customer_name',
            'brand_name', 'licenses', 'is_active', 'created_at', 'updated_at'
        ]
