...
class BrandAPIKeyAuthentication(authentication.BaseAuthentication):
    ...
    def authenticate(self, request):
        ...
        try:
            ...
            logger.warning(f"Brand authentication failed: invalid API key")
            raise exceptions.AuthenticationFailed('Invalid API credentials')
        
        # Verify secret
        if brand.api_secret != api_secret:
            logger.warning(f"Brand authentication failed: invalid secret for brand {brand.slug}")
            raise exceptions.AuthenticationFailed('Invalid API credentials')
...
class LicenseKeyAuthentication(authentication.BaseAuthentication):
    ...
    def authenticate(self, request):
        ...
        try:
            ...
            logger.warning(f"License key authentication failed: invalid key")
